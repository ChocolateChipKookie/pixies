...
#include "PerlinNoisedPlanktonPixies.h"
#include <glm/glm.hpp>

PerlinNoisedPlanktonPixies::PerlinNoisedPlanktonPixies(kki::Texture* background, size_t count, float max_speed, float acceleration,
    float noise_scale, float random_noise, bool random_start)
...
PerlinNoisedPlanktonPixies::PerlinNoisedPlanktonPixies(kki::Texture* background, const PerlinNoisedPlanktonPixies::Setup& setup)
    :   PerlinNoisedPlanktonPixies(background, setup.count, setup.max_speed, setup.acceleration, setup.noise_scale, setup.random_noise, setup.random_start)
{}
void PerlinNoisedPlanktonPixies::update(size_t i)
{
    ...
    //if ( !kki::inRange(-0.95f, 0.95f, pos.x) || !kki::inRange(-0.95f, 0.95f, pos.y)){
